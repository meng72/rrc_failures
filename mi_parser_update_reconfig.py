...
class MiParserUpdateReconfig:
            ...
            for line in f:
                ...
                if "</dm_log_packet>" in line:
                    ...
                    if packet_type == "LTE_RRC_OTA_Packet":
                        if 'c1: rrcConnectionReconfiguration (4)' in dm_log_packet_str:
                            #print dm_log_packet_str
                            extra = {}
                            for pair in pairs:
                                if pair.get('key') == 'Msg':
                                    #reconfig_list = pair.findall('./msg/packet/proto/field/field/field')
                                    reconfig_list = pair.findall('.//')
                                    for reconfig in reconfig_list:
                                        if reconfig.get('showname') == 'c1: rrcConnectionReconfiguration (4)':
                                            #config_list = reconfig.findall('./field/field/field/field/field/field')
                                            config_list = reconfig.findall('.//')
                                            for config in config_list:
                                                extra = self.get_config_for_reconfig(config, extra)

                            #print extra
                            db.execute(''' update messages set extra = "%s" 
                                where user_id = %d and time = %d and message_type_id = 23 
    ...
    def get_config_for_reconfig(self, config, extra):
